...
@section('title', 'Editar Historia de Piel')
...
<div class="max-w-2xl mx-auto mt-10">
    ...
    <div class="text-center mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">✏️ Editar Historia de Piel</h1>
        <p class="text-gray-600">Modifica los datos de tu historia dermatológica y adjunta archivos actualizados</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-8">
            <form action="{{ route('historias.update', $historia->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    @if($historia->foto_perfil)
                        <img src="{{ asset('storage/' . $historia->foto_perfil) }}" alt="Foto actual" class="mt-2 w-20 h-20 object-cover rounded">
                    @endif
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="dermatologo_id" id="dermatologo_id"
                        ...
                        @foreach($dermatologos as $derma)
                            <option value="{{ $derma->id }}" {{ $historia->dermatologo_id == $derma->id ? 'selected' : '' }}>
                                {{ $derma->name }}
                            </option>
                        @endforeach
                    </select>
